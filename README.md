...
本插件**默认使用** [OpenCLI](https://github.com/jackwener/OpenCLI) 进行翻译。OpenCLI 通过浏览器自动化实现 Bing 翻译功能，无需配置 API Token，开箱即用。

### 前置要求

1. **安装 Node.js**：确保已安装 Node.js（推荐 v16+）
2. **安装 Chrome 浏览器**：OpenCLI 需要使用 Chrome 进行浏览器自动化
...
如果 `opencli doctor` 提示浏览器桥接未连接，需要先启动 Chrome：
...
脚本会自动将 `opencli/` 目录下的插件文件复制到 `~/.opencli/clis/my-bing/` 目录。
...
# 复制所有 .js 文件
Copy-Item -Path ".\opencli\*.js" -Destination "$env:USERPROFILE\.opencli\clis\my-bing\" -Force
```
...
# 查看不同格式输出
opencli my-bing do-translate "Test" -f json
opencli my-bing do-translate "Test" -f yaml
opencli my-bing do-translate "Test" -f md
```

### VSCode 插件配置

VSCode 插件默认使用 OpenCLI 服务，无需额外配置。如需切换到 Bing API 模式，可在 VSCode 设置中添加以下配置：

```json
{
  "realtimeTranslate.translationService": "bing",
  "realtimeTranslate.token": "your-bing-api-token"
}
```

### OpenCLI vs Bing API 对比

| 特性 | OpenCLI（默认） | Bing API |
|------|----------------|----------|
| 配置复杂度 | 无需 Token，需安装 Chrome | 需要 API Token |
| 首次翻译速度 | 较慢（需启动浏览器） | 较快 |
| 连续翻译速度 | 快（复用浏览器会话） | 稳定 |
| 稳定性 | 依赖浏览器和网络 | 更稳定 |
| 适用场景 | 个人使用、开发环境 | 生产环境、团队使用 |
...
或检查插件是否正确安装到 `~/.opencli/clis/my-bing/` 目录。
...
更多 OpenCLI 使用说明请参考：[opencli/README.md](./opencli/README.md)
